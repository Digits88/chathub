...
if LOG_CONFIG_FILE is not None:
    ...
    if 'ROS_LOG_FILENAME' not in os.environ:
        os.environ['ROS_LOG_FILENAME'] = os.path.join(
            os.environ['ROS_LOG_DIR'], 'chathub_server.log')
    logging.config.fileConfig(LOG_CONFIG_FILE)
else:
    log_dir = os.path.expanduser('~/.hr/log')
    if not os.path.isdir(log_dir):
        os.makedirs(log_dir)
    LOG_CONFIG_FILE = '{}/chathub_server_{}.log'.format(log_dir,
            dt.datetime.strftime(dt.datetime.now(), '%Y%m%d%H%M%S'))
    link_log_fname = os.path.join(log_dir, 'chathub_server_latest.log')
...
logger = logging.getLogger('hr.chathub.server')
